...
option '-t', '--tools', 'convert the json.coffee files
  and install the npm and bower deps then start'
option '-p', '--pyeth', 'install pyethereum and serpent'
option '-n', '--nobuild', 'skips building brunch in cake:start'
option '-w', '--watch', 'compile app and start nw, watching the src for changes: sudo cake -w start'
appName = 'eth-ui'
...
installJSON = (cb) ->
    log 'If this fails do a manual bower install and/or npm install'
    pcc = 0 # process completion count, 2 when both are finished
    run 'npm', ['install'], su, ->
        log 'Installed npm deps'
        pcc += 1
        log 'If this fails do a manual bower install and/or npm install'
        if pcc is 2 and cb then cb()
    run 'bower', ['install'], {uid: 1000}, ->
        log 'Installed bower deps dependencies'
        pcc += 1
        log 'If this fails do a manual bower install and/or npm install'
        if pcc is 2 and cb then cb()
...
startNWgulp = (cb) ->
    ...
    run 'gulp', ['nw'], ->
        log 'Done, started server'
        if cb then cb()
#Async translate and install bower and npm package
installDeps = ->
    pcc = 0
    fromCSON 'bower', 'cson', ->
        pcc += 1
        if pcc is 2 then installJSON()
    fromCSON 'package', 'cson', ->
        pcc += 1
        if pcc is 2 then installJSON()

task 'install', 'install pyethereum, serpeant, bower & npm deps', (options) ->
                ...
                fromCSON 'nw', 'cson', 'public', 'package', ->
                    ...
                    run 'rm', ['./public/js/dontUseMe', './public/js/dontUseMe.map'], su, ->
                        ...
                        if nwi isnt -1 # Remove any old nw existence ref
                            files.slice nwi, 1
                        files.unshift appName + '.nw' # add output name for zip command
